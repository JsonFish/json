...
import { deepClone } from '@/utils/tool.ts'
...
const search = async (type) => {
  ...
  if (res.code == 200) {
    ...
    keyWordsSongs.value =
      params.offset == 0 ? list : keyWordsSongs.value.concat(list)
    params.loading = false
    if (!keyWordsSongs.value.length) {
      params.loadMore = false
      ElMessage({ type: 'info', message: '没有相关的歌曲' })
      return
    }
    if (type !== 'loadMore') {
        ...
        <el-row v-if="keyWordsSongs.length" class="body">
          ...
          >
            ...
            <div class="author">
              <span
                class="text-overflow"
                :title="returnAuthors(item.artists, 'name')"
                >{{ returnAuthors(item.artists, 'name') }}</span
              >
            </div>
            <div class="add-music">
              <i
