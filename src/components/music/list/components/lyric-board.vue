...
import { gsapTransLyric, gsapTransLyricLeftToRight } from '@/utils/transform.ts'
...
const calcLyricDuration = () => {
  const nextTime =
    getMusicInfo.value.lyricTimeList[getCurrentLyricIndex.value + 1]
  const currentTime =
    getMusicInfo.value.lyricTimeList[getCurrentLyricIndex.value]
...
<template>
  <div
    :class="[
      'lyric-mask',
      getShowLyricBoard ? 'lyric-mask-show' : 'lyric-mask-hide',
    ]"
    :style="{
      ...
      <div id="lyricBox" class="right" @scroll="lyricScroll">
        <div class="!p-[10px]">
          ...
          <div
            ...
            :class="[
              'lyric-word',
              getCurrentLyricIndex == index ? 'current' : '',
            ]"
    ...
    >
      <div class="special-title">
        <div class="flex items-center justify-center">
          <span
            class="small-action"
            @click="changeSpecialTitleSize(false)"
          ></span>
          <SpecialTitle
          ...
          <span
            class="small-action"
            @click="changeSpecialTitleSize(true)"
          ></span>
        </div>
    ...
    >
      ...
      >
        <div class="close-board">
          <i
            class="iconfont icon-arrowdown change-color mr-[10px]"
            @click="closeBoard"
          ></i>
          <i
            v-show="getLyricType == 'SPECIAL'"
        ...
        <div class="control-group">
          ...
          <i
            class="iconfont icon-zanting change-color"
            v-if="getIsPaused"
            @click="play"
          ></i>
          <i
            class="iconfont icon-bofangzhong change-color"
            v-else
            @click="play"
          ></i>
