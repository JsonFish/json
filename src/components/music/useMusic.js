...
const setters = {
  ...
  init() {
    ...
    audio.ontimeupdate = () => {
      ...
      if (!this.isClickLyric) {
        let index = this.musicInfo.lyricTimeList.findIndex(
      ...
      if (!this.isUseProgress) {
        this.currentSchedule = calcMusicSchedule(
          audio.currentTime,
          audio.duration,
        )
      }
  ...
  setCustomerMusicList(type, music) {
    if (type == 'add') {
      ...
      let index = this.customerMusicList.findIndex(
